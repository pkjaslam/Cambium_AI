...
## ADR-015 - Claim-grain provenance lineage convention (PROV-AGENT ADAPT) -- PROPOSED
- Status: Proposed (gated — do NOT implement without human approval).
- Context: the 2026-06-26 2341 self-improve run looked at the agentic-provenance / reproducibility-of-runs slice. Grounding (`toolsmith.py`, `task_router.py`, `grep lineage|provenance|root-cause`) confirmed Cambium has provenance only at two coarse grains: `governance/provenance.json` (per-deliverable manifest of models + agents + evidence tier) and `tools/run_trace.py` (live execution board of which council/agents work now). Neither links an individual claim/number in the output to the specific agent decision / prompt / input that produced it. PROV-AGENT (Souza et al., IEEE e-Science 2025; arXiv 2508.02866; code ORNL/flowcept) makes agent tools/prompts/responses/decisions first-class W3C-PROV nodes so a final claim can be traced back to its first input (their Q1–Q5 root-cause queries). Its Flowcept/Redis/Kafka/HPC infrastructure is far heavier than a framework needs — reject the install; adopt only the schema idea. Sibling candidates this run: R-LAM (arXiv 2601.09749) SKIP (duplicates exec-iteration + research-engineer + OUTPUT_CONTRACT); HAL harness (princeton-pli/hal-harness) SKIP (eval leaderboard, out of single-project lane; referee + EVALS already own evaluation; cf. MCPEval SKIP).
- Decision (PROPOSED, framework-only; no third-party content copied; nothing installed; counts stay 46·11·8): (1) propose a lightweight, infra-free claim-lineage convention — extend the provenance manifest with an optional `claims[]` array where each headline claim records `claim_id`, `evidence_tier` (Proved/Code-verified/Asserted/Open), `generated_by` (council · agent), `used` (input artifact/dataset/citation id), optional `informed_by` (prompt/decision ref); draft first as a `templates/` doc, not a schema/`validate.py` change. (2) propose a future read-only "lineage / root-cause" helper that answers Q1/Q5 from `claims[]` — recorded as a proposal only, not built. (3) reuse-first guardrail: must wrap existing `provenance.json` + `run_trace.py`; no new provenance engine, no Flowcept/Redis/Kafka, no re-architecture. Natural owners: record-keeper (maintains rows) + Verification council (consumes for root-cause). Pairs with the still-open supersession concern (the two-`ADR-008` defect): claim-grain lineage is the same machinery that catches a silent supersession.
